...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_WAIT: {
		...
		{
			current->waitingFor = &proc_array[p];
...
do_fork(process_t *parent)
{
  ...
  for(i = 1; i < NPROCS; i++)
    {
      //find the first empty process and activate it.
      if(proc_array[i].p_state == P_EMPTY)
	{
	  ...
	  //return 0 in child's context.
	  proc_array[i].p_registers.reg_eax=0;
	  //return 1 in parent's context.
	  return i;
	}
    }
...
copy_stack(process_t *dest, process_t *src)
{
	...
	pid_t src_id = src->p_pid;
	pid_t dest_id = dest->p_pid;

	//calculate stack top for src by adding to the stack top
	//the product of the maximum stack size of each stack and 
	//the id of src, which is the address of the top of the stack
	//of process src.
	src_stack_top = PROC1_STACK_ADDR + PROC_STACK_SIZE * src_id;

	//the bottom of stack bottom for src is pointed to by the 
	//stack pointer associated to src.
	src_stack_bottom = src->p_registers.reg_esp;

	//calculate the size of the stack of src, which will be used
	//in memcpy().
	size_t stackSize = src_stack_top - src_stack_bottom;

	//do the same thing above to dest stack
	dest_stack_top = PROC1_STACK_ADDR + PROC_STACK_SIZE * dest_id;

	//copy the stack
	memcpy((void*)dest_stack_top, (void*)src_stack_top, stackSize);

	//assign the correct value of dest's stack pointer
	//(to its stack bottom)
	dest->p_registers.reg_esp = dest_stack_top - stackSize;

	//src_stack_top = 0 /* YOUR CODE HERE */;
	//src_stack_bottom = src->p_registers.reg_esp;
	//dest_stack_top = 0 /* YOUR CODE HERE */;
	//dest_stack_bottom = 0 /* YOUR CODE HERE: calculate based 
...
schedule(void)
{
	...
	while (1) {
		...
		if (proc_array[pid].p_state == P_RUNNABLE)
			...
			proc_array[pid].waitingFor->p_state == P_ZOMBIE)
		{
			proc_array[pid].p_state = P_RUNNABLE;
			current->p_registers.reg_eax = 
				current->waitingFor->p_exit_status;
			current->waitingFor->p_state = P_EMPTY;
			run(&proc_array[pid]);
		}
	}
}
